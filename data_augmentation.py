...
#using train, and test directories
...
def make_dataframes(train_dir, test_dir):
    ...
    for name,d in zipdir:
        ...
        if name =='test':
            ...
            pdf=df 
    #From the 'train' directory we randomly take 5% of the image for the validation of the xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx
    train_df, valid_df=train_test_split(pdf, train_size=.95, shuffle=True, random_state=123, stratify=pdf['labels']) 


    classes=sorted(train_df['labels'].unique())#find the classes in the index 
    class_count=len(classes)#nbr of classes
...
train_dir = r'C:\Users\tgillard\Desktop\internship\DATA_SET\T1 Braeburn_trie\Team\train'
test_dir=r'C:\Users\tgillard\Desktop\internship\DATA_SET\T1 Braeburn_trie\Team\test'

train_df, test_df, valid_df, classes, class_count=make_dataframes(train_dir, test_dir)

#Trim train_df so no class has more than 280 images                !!  CHANGEMENT POSSIBLE !!
#This will balance the dataset
...
# use the Braeburn_21_11-97.19.h5 model for the final test
#model.load_weights(r'C:\Users\tgillard\Desktop\internship\0_Final_Code\1_Braeburn\Braeburn_tesy.h5') # load the weights from the best model
#model = model.load_weights(r'C:\Users\tgillard\Desktop\internship\0_Final_Code\1_Braeburn\Braeburn_tesy.h5') # load the weights from the best model
...
class LR_ASK(keras.callbacks.Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):  # method runs on the end of each epoch
        ...
        if epoch >0:
            ...
            deltaa=self.lowest_aloss-aloss #xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx on aloss
...
#2   https://www.youtube.com/watch?v=2U6Jl7oqRkM
tensorboard = TensorBoard(log_dir="logs/{}".format(time()))


history=model.fit(x=train_gen,  epochs=epochs, verbose=1, callbacks=[tensorboard],  validation_data=valid_gen,
...
if len(error_list) > 0 and len(error_list)<200:
